...
def _ensure_mlflow(tracking_uri: Optional[str], experiment: Optional[str], *, load_env: bool = False) -> None:
    ...
    uri = tracking_uri or os.getenv("CENTRALPROMPT_MLFLOW_TRACKING_URI") or os.getenv("MLFLOW_TRACKING_URI")
    if uri:
        mlflow.set_tracking_uri(uri)
    exp = experiment or os.getenv("CENTRALPROMPT_MLFLOW_EXPERIMENT_NAME") or os.getenv("MLFLOW_EXPERIMENT_NAME")
    if exp:
...
def _ensure_langfuse(load_env: bool = True):
    ...
    if load_env:
        try:
            ...
            pass
    # Prefer CENTRALPROMPT_LANGFUSE_* env vars; set standard LANGFUSE_* for the SDK.
    cp_public = os.getenv("CENTRALPROMPT_LANGFUSE_PUBLIC_KEY")
    cp_secret = os.getenv("CENTRALPROMPT_LANGFUSE_SECRET_KEY")
    # Support both HOST and BASE_URL naming; prefer BASE_URL if provided
    cp_base_url = os.getenv("CENTRALPROMPT_LANGFUSE_BASE_URL")
    cp_host = os.getenv("CENTRALPROMPT_LANGFUSE_HOST")

    if cp_public is not None:
        os.environ["LANGFUSE_PUBLIC_KEY"] = cp_public
    if cp_secret is not None:
        os.environ["LANGFUSE_SECRET_KEY"] = cp_secret
    # Set both to maximize compatibility across langfuse client versions
    resolved_host = cp_base_url or cp_host
    if resolved_host is not None:
        os.environ["LANGFUSE_BASE_URL"] = resolved_host
        os.environ.setdefault("LANGFUSE_HOST", resolved_host)
